    ...
    <form action="{{ route('articles.update', $article->id) }}" method="POST" class="max-w-md mx-auto">
        ...
        <div class="mb-5">
            ...
            <textarea id="excerpt" rows="4" name="excerpt"
                ...
                placeholder="Excerpt">{{ $article->excerpt }}</textarea>
            @error('excerpt')
                <span class="text-red-600 text-s">{{ $message }}</span>
            @enderror
        </div>
        <div class="mb-5">
            ...
            <textarea id="context" rows="10" name="context"
                class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                placeholder="Context">{{ $article->context }}</textarea>
            @error('context')
                <span class="text-red-600 text-s">{{ $message }}</span>
            @enderror
        </div>
        ...
        <button type="submit"
            class="focus:outline-none text-white bg-cyan-700 hover:bg-cyan-800 focus:ring-4
             focus:ring-cyan-300 font-medium rounded-lg text-sm px-10 py-2.5 mt-5 dark:bg-cyan-600
              dark:hover:bg-cyan-700 dark:focus:ring-cyan-800">Update</button>
    </form>
</body>
