...
<body>
    <form action="{{ route('products.update', $product->id) }}" method="POST" class="max-w-md mx-auto mt-10">
        @csrf
        <div class="mb-5">
        ...
        <div>
            ...
            <select id="status" name="status"
                ...
                <option value="0" @if ($product->status === 0) selected @endif>Completed</option>
                <option value="1" @if ($product->status === 1) selected @endif>Incomplete</option>
            </select>
        </div>
        <button type="submit"
            class="focus:outline-none text-white bg-cyan-700 hover:bg-cyan-800 focus:ring-4 focus:ring-cyan-300 font-medium rounded-lg text-sm px-10 py-2.5 mt-5 dark:bg-cyan-600 dark:hover:bg-cyan-700 dark:focus:ring-cyan-800">Update</button>
    </form>
</body>
